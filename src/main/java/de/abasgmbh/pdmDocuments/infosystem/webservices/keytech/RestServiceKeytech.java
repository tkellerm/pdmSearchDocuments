...
public class RestServiceKeytech extends AbstractRestService {
	...
	public String searchPdmProductID(String abasIdNo) throws PdmDocumentsException {
		...
		try {
			...
			throw new PdmDocumentsException(
					Util.getMessage("pdmDocument.restservice.keytech.error.jsonToObject", "ResponsePDMProductId"), e);
	...
	private PDMLinks searchProductLinks(String pdmProductID) throws PdmDocumentsException {
		...
		try {
			...
			throw new PdmDocumentsException(
					Util.getMessage("pdmDocument.restservice.keytech.error.jsonToObject", "PDMLink"), e);
		} catch (JsonMappingException e) {
			throw new PdmDocumentsException(
					Util.getMessage("pdmDocument.restservice.keytech.error.jsonToObject", "PDMLink"), e);
		} catch (IOException e) {
	...
	// private List<File> getPdmFile(FileInfo fileInfo, ChildLink childLink)
	// throws PdmDocumentsException {
	//
	//
	//
	// // String targetFileName=
	// // Util.replaceUmlaute(fileInfo.getFilename().replaceAll(" ", "_"));
	// String targetFileName =
	// Util.replaceUmlaute(fileInfo.getFileDisplayname().replaceAll(" ", "_"));
	//
	// try {
	// List<File> fileList = downloadFileFromRestservice(url, targetFileName,
	// getTargetPath());
	...
	// log.error(e);
	// throw new
	// PdmDocumentsException(Util.getMessage("pdmDocument.restservice.keytech.error.getfilefromService"),
	// e);
	// } catch (IOException e) {
	// log.error(e);
	// throw new
	...
	private ArrayList<FileInfo> getFileInfoList(ChildLink childLink) throws PdmDocumentsException {
		...
		try {
			...
			log.error(e);
			throw new PdmDocumentsException(
					Util.getMessage("pdmDocument.restservice.keytech.error.jsonToObject", "FileInfoList"), e);
		} catch (JsonMappingException e) {
			log.error(e);
			throw new PdmDocumentsException(
					Util.getMessage("pdmDocument.restservice.keytech.error.jsonToObject", "FileInfoList"), e);
